...
**Only run small, read-only sample queries.** You are inspecting data to write metadata — you never need bulk exports.

- ALWAYS use `LIMIT` on every query (10–25 rows max)
- NEVER run `SELECT *` — select only the columns you're investigating
- NEVER export, copy, or store raw data — only write metadata (descriptions, roles, rules)
- NEVER put PII (emails, names, SSNs, phone numbers, addresses) in `sample_values` — use redacted examples like `"user@***.com"` or describe the pattern instead (e.g., `"email addresses"`)
- If you discover PII columns, flag them in governance with `security: pii` and suggest a guardrail filter
