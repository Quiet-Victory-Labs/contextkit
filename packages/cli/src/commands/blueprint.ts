...
export const blueprintCommand = new Command('blueprint')
  .description('Export data product YAML blueprints following the OSI specification')
  ...
  .action(async (modelArg, opts) => {
    try {
      ...
      if (!opts.stdout) {
        const outDir = path.resolve(opts.out);
        console.log(
          chalk.blue(`\n${modelNames.length} blueprint(s) written to ${outDir}/`),
        );
      }
    } catch (err) {
...
function emitBlueprint(name: string, manifest: Manifest): string {
  ...
  lines.push(`# Portable data product blueprint — Open Semantic Interchange (OSI) v1.0`);
