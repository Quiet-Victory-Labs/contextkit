...
The RunContext CLI is powerful for solo work — you build a semantic plane on your machine and serve it locally. But when your team needs to share that context, things get complicated:
...
<CardGrid>
  ...
  <Card title="Guardrailed MCP Connectors" icon="laptop">
    Connect databases through RunContext's managed MCP connectors — PostgreSQL, Snowflake, BigQuery, and more. Unlike generic MCP database tools that may expose write access or unrestricted queries, RunContext connectors are strictly read-only. They introspect your schema and serve context to AI agents, nothing else. No mutations, no data extraction, no risk.
  </Card>
</CardGrid>
...
You still use the RunContext CLI to build your semantic plane. Nothing changes about your local workflow. When you're ready to share, publish:
...
**Connectors** — Set up guardrailed MCP database connectors. Choose which tables to introspect and how often to refresh. Strictly read-only — no write access, no arbitrary queries.
...
<Aside type="tip">
You don't need a cloud account to use RunContext. The CLI is free and runs entirely on your machine. Cloud is for teams that want shared access and managed infrastructure.
</Aside>
...
| **MCP connectors** | — | 3 | Unlimited |
| **Team seats** | 1 (local) | 5 (+$10/seat) | Unlimited |
| **RBAC** | — | — | Yes |
| **SSO** | — | — | Yes |
| **Support** | GitHub Issues | Email | Priority Slack |

See [Pricing](https://runcontext.dev/pricing/) for full details and FAQ.
