---
title: MCP Resources
description: Read-only resource URIs exposed by the RunContext MCP server.
---
...
Returns a blank AI Blueprint template — an OSI-compliant YAML starting point for a new semantic plane.

**When to use:** When starting a new semantic plane from scratch. The template includes all OSI sections with comments explaining each field.
...
Returns the **AI Blueprint** for a semantic plane — the complete semantic specification as portable OSI YAML. Includes the model's schema, governance metadata, glossary terms, metrics, relationships, business rules, guardrail filters, and golden queries — everything an AI agent needs to work with this data correctly.

**When to use:** When you need to share a semantic plane with another team, feed it to an AI agent, migrate to a different warehouse, or create a portable backup of your semantic layer.
...
For portability, agents can read any semantic plane's AI Blueprint via `context://data-product/{name}`, or scaffold a new one from the blank template at `context://data-product/template`.
