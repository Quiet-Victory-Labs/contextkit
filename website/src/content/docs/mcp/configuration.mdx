...
description: How to configure the RunContext MCP server for Claude Code, Cursor, VS Code, Windsurf, OpenAI Codex, and other MCP clients.
---

import { Tabs, TabItem, Aside } from '@astrojs/starlight/components';

The RunContext MCP server can be configured for any MCP-compatible client. You can run it **locally** (stdio or self-hosted HTTP) or connect to the **hosted endpoint** at `api.runcontext.dev/mcp`.
...
For the hosted endpoint, replace `{org}` with your organization slug and set `RUNCONTEXT_API_KEY` to your API key (available from the RunContext dashboard).
...
```json title=".claude/mcp.json"
{
  "mcpServers": {
    "contextkit": {
      "url": "https://api.runcontext.dev/mcp/{org}",
      "headers": {
        "Authorization": "Bearer ${RUNCONTEXT_API_KEY}"
      }
    }
  }
}
...
```json title="~/.codeium/windsurf/mcp_config.json"
{
  "mcpServers": {
    "contextkit": {
      "serverUrl": "https://api.runcontext.dev/mcp/{org}",
      "headers": {
        "Authorization": "Bearer ${RUNCONTEXT_API_KEY}"
      }
    }
...
Once configured, your AI tool should discover the RunContext resources and tools automatically. You can verify by asking the agent:
