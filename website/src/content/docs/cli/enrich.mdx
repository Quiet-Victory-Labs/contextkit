...
> **Note:** In the RunContext UI, enrichment is now called **Curate** and is agent-driven. The `context enrich` CLI command still works for scripted/CI workflows, but the recommended flow for interactive use is agent-driven curation via the MCP server or `context setup`.
