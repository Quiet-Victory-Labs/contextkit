...
The **Open Semantic Interchange (OSI)** specification is an open standard for describing semantic models in a vendor-neutral format. OSI v1.0 is backed by Snowflake, dbt Labs, Salesforce, and other industry participants. RunContext reads and validates OSI files natively.
...
RunContext uses OSI as the foundation layer. All schema information -- datasets, fields, relationships, and metrics -- lives in `*.osi.yaml` files. Governance, business rules, and enrichment live in separate companion files, so the OSI files remain portable and standard-compliant.
...
RunContext treats OSI files as read-only schema definitions. When you run `context introspect`, RunContext generates OSI files from your warehouse. When you run `context enrich`, RunContext writes governance and rules to companion files -- never to the OSI file itself.

This means your OSI files remain compliant with the specification and portable across tools. Any enrichment RunContext adds is stored separately and can be stripped away without affecting the core schema definition.
